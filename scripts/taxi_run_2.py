...
if __name__ == '__main__':
    ...
    for epsilon in [0.1, 0.1]:
        for _ in range(40):
            ...
            Z = np.dot(np.eye(sa_idx[0].shape[0]) - mdp_wrap.gamma * np.dot(estimator_ex.P[sa_idx],
                                                                            policy.pi2[:, sa_idx].squeeze()), phi)
            psi_mb = la2.range(Z)
            ...
            re_state = RelativeEntropyIRL(eco_state_features_mf,#state_features,
                                         trajectories_ex,
                                         trajectories_random,
                                         mdp_wrap.gamma,
                                         mdp_wrap.horizon,
            ...
            for i in range(len(scaled_basis_functions)):
                ...
                kl = []
                for j in range(0, iterations+1, 15):
                    policy.set_parameter(history[j][0])
                    d = kullback_leibler_divergence(
                        expert_policy.pi[:n_states - 1], policy.pi[:n_states - 1])
                    kl.append(d)
                    #print(d)
        ...
        np.save('me_histories_%s' % epsilon, histories)
        np.save('me_kls_%s' % epsilon, kls)
